...
export default function App() {
  ...
  const persist = useCallback((nd) => { setData(nd); saveData(nd); try { localStorage.setItem("lab_data_backup", JSON.stringify(nd)); } catch{} }, []);
  ...
  useEffect(() => {
    loadData().then(d => {
      if (d && Object.keys(d).length > 0) {
        ...
        const existing = localStorage.getItem("lab_data_backup");
        if (existing) {
          try { setData(JSON.parse(existing)); } catch { setData(makeDefault()); }
        } else {
          setData(makeDefault());
        }
      }
